...
## example 3: grounded cognition (Brandt et al.)
...
Brandt.sd2 <- 152.03
Brandt.d <- .03
Brandt.n1 <- 60 # this is a guess, the paper only reports total N = 121
Brandt.n2 <- 61 # see previous comment

## v1: testing equivalence to original effect
# v1.1: equivalence bounds identical to H1 model suggestions by Dienes & McLatchie,
# i.e. lower bound = 0, upper bound = twice the original effect size
TOST.Brandt.1.1 <- TOSTtwo(m1=Brandt.m1, m2=Brandt.m2, sd1=Brandt.sd1, sd2 = Brandt.sd2, 
                       n1 = Brandt.n1, n2=Brandt.n2, low_eqbound_d = 0, high_eqbound_d = 2*Banerjee.d, 
                       var.equal = TRUE, prior_dist = "halfnormal", effect_prior = Banerjee.d)

# v1.2: small telescopes approach (lower bound: d for which original had 33% power)
TOST.Brandt.1.2 <- TOSTtwo(m1=Brandt.m1, m2=Brandt.m2, sd1=Brandt.sd1, sd2 = Brandt.sd2, 
                         n1 = Brandt.n1, n2=Brandt.n2, low_eqbound_d = 0.35, high_eqbound_d = 0.99, 
                         var.equal = TRUE, prior_dist = "halfnormal", effect_prior = Banerjee.d)

# v1.3: critical d approach (lower bound: smallest d the original study could have detected)
# calculate critical d
crit_t = qt(1-.05/2, Banerjee.N-2)
crit_d = crit_t*sqrt(1/(Banerjee.N/2) + 1/(Banerjee.N/2))

TOST.Brandt.1.3 <- TOSTtwo(m1=Brandt.m1, m2=Brandt.m2, sd1=Brandt.sd1, sd2 = Brandt.sd2, 
                         n1 = Brandt.n1, n2=Brandt.n2, low_eqbound_d = crit_d, high_eqbound_d = Banerjee.d+(Banerjee.d-crit_d), 
                         var.equal = TRUE, prior_dist = "halfnormal", effect_prior = Banerjee.d)


## v2: testing if replication effect is too small to care about
# v2.1: small telescopes (centred on zero)
TOST.Brandt.2.2 <- TOSTtwo(m1=Brandt.m1, m2=Brandt.m2, sd1=Brandt.sd1, sd2 = Brandt.sd2, 
                           n1 = Brandt.n1, n2=Brandt.n2, low_eqbound_d = -0.35, high_eqbound_d = 0.35, 
                           var.equal = TRUE, prior_dist = "halfnormal", effect_prior = Banerjee.d)

# v2.3: critical d approach (lower bound: smallest d the original study could have detected)
# calculate critical d
crit_t = qt(1-.05/2, Banerjee.N-2)
crit_d = crit_t*sqrt(1/(Banerjee.N/2) + 1/(Banerjee.N/2))

TOST.Brandt.2.3 <- TOSTtwo(m1=Brandt.m1, m2=Brandt.m2, sd1=Brandt.sd1, sd2 = Brandt.sd2, 
                           n1 = Brandt.n1, n2=Brandt.n2, low_eqbound_d = -crit_d, high_eqbound_d = crit_d, 
                           var.equal = TRUE, prior_dist = "halfnormal", effect_prior = Banerjee.d)
