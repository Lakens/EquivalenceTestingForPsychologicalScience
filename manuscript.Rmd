...
We find that the test against $\Delta_{upper}$ is `r ifelse(Lynott$TOST_p2 < Lynott$alpha, "significant", "non-significant")`, $z =$ `r round(Lynott$TOST_z2, 2)`, $p =$ `r ifelse(round(Lynott$TOST_p2, 3) >= 0.001, paste("=", round(Lynott$TOST_p2, 2)), "< 0.001")` (see figure 2D). Thus, we can conclude that the statistically non-significant effect is also statistically inferior to our SESOI.
...
For instance, in example 3 we showed that Moon & Roeder's replication of the identity-priming study by Shih et al. (1999; example 3) does not exclude effects that are large enough to affect a student's final grade on a test. However, if we look at the $90 \%$ CI (figure XXX), we can see that the data would have been statistically equivalent if the bounds had been set to  a slightly larger SESOI, e.g. a mean difference of 0.1.
...
In order to avoid fooling ourselves more often than $\alpha \%$ of the time, equivalence bounds should be specified before results are known (ideally as part of a preregistration; cf. CONSORT guidelines, Piaggio et al., 2006). A perhaps helpful guiding question may be: "Will testing against these bounds yield interesting information in the long run?" 


#Justify your equivalence bounds

Like with any statistical test, the usefulness of the result an equivalence test gives us depends on the question we ask. The question manifests in the bounds we set: Is our data surprising if the true effect lies outside of the bounds? If we set very wide bounds, we ask if our data is surprising given that the true effect is very large[^footnote]. For most psychological effects, the answer will usually be “yes,” which means that it may not be a very interesting question to ask. In order to obtain a meaningful result, we have to set the bounds based on meaningful criteria: Our SESOI should really be *the smallest effect we are interested in* for a given study.  For this reason, researchers should provide a detailed justification of the choice for a SESOI. Some studies in the literature simply state the SESOI that is used, without providing a justification. For example, Brown and colleagues (2017) and Schumann and colleagues (2017)use a SESOI of Cohen’s d = +/- 0.5, but it is not clear why these bounds were chosen[^footnote]. An equivalence test should answer a question, and justifying the SESOI is an essential part of asking the question. An equivalence test of d = 0.5 might very well answer a question the researchers are interested in (for one possible justification based on minimally important differences, see Norman et al., 2003), but the reason this SESOI was chosen should be explicitly justified. 


[^footnote]: To be precise, wide bounds set symmetrically around 0 ask if the data is surprising given that the true effect is very *extreme*, i.e. either very large (positive) or very small (negative).
