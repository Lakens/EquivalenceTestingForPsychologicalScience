...
eqboundCritera <- function(n1, n2, orig.df, p = 0.05, power = 0.33, testtype = "two.sided", var.equal = FALSE) {
  ...
  output$dunb33 <- ( 1 - ( 3 / (4*orig.df - 1) ) ) * output$d33     # Calculate d unbiased for the 33% power criterion
  
  
  ## Original study smallest detectable effect size ##
  
  output$crit.t  <- qt(p = 1-p/2, df = orig.df) # Critical t for original study
